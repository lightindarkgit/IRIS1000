...
返回值：
    E_PWM_SUCCESS(0) -- 成功
**********************************/
int ptzBase::setSteeringAngle(int angle, bool homing)
{
...
int ptzBase::setSteeringFrame(float offsetAngle, int id)
{
    ...
    int curAngle = pwmGetCurAngle();
    int setAngle = curAngle + (int)offsetAngle;
    bool homing = false;
...
返回值：
    舵机当前角度
**********************************/
int ptzBase::getSteeringAngle()
{
    int angle = pwmGetCurAngle();
