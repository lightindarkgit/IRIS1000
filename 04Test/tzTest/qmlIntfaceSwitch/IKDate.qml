...
Rectangle {
    ...
    anchors.fill: parent
    color: "#80000000"
    property var currentData:"2017-11-29"
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            width: 100
            height: 400
            x: (contain.width - year.width - month.width - day.width) /2 - 85
            ...
            PathView {
                ...
                model: ListModel {
                    Component.onCompleted: {
                        ...
                        //                console.log("fdefff" + Date.fromLocaleString(locale,dateTimeString, "yyyy-MM-dd"));
                        var tmp = Date.fromLocaleString(locale, dateTimeString, "yyyy-MM-dd").toString().slice(0,2)
                ...
                onMovementEnded:{
                    ...
                    //            showDate.text = showYear + showMonth + showDay + dateTime.toString().slice(0,2);
                    //            console.log("fe " + dateTime.toString().slice(0,2), + " a" + dateTime)
                    var locale= Qt.locale()
                    var dateTimeString = yearData[curYear]+"-"+ monthData[curMonth]+"-"+ dayData[curDay]
                    console.log("fdefff" + Date.fromLocaleString(locale,dateTimeString, "yyyy-MM-dd"));
                    var tmp = Date.fromLocaleString(locale, dateTimeString, "yyyy-MM-dd").toString().slice(0,2)
        ...
        Rectangle {
            ...
            PathView {
                ...
                onMovementEnded:{
                    ...
                    console.log("month  " + monthData[curMonth])
                    var taD = dayD[curMonth]
                    if(taD < dayData[curDay] )
                    {
                        dayData.splice(0, dayData.length);
        ...
        Rectangle {
            ...
            width: 100
            height: 400
            //    x: month.x + month.width + 30
            x: monthText.x + monthText.width + 130
            y: contain.height - 400/*year.height*/ - date_cancel.height
            //    color: "#01010a"
            Component {
                id: delegateD
                Column {
            ...
            PathView {
                ...
                onMovementEnded:{
                    curDay = currentIndex
                    console.log("change day  " + curDay + "  " + dayData[curDay])
                    showDay = dayData[curDay] + "日"
                    var locale= Qt.locale()
                    var dateTimeString = yearData[curYear]+"-"+ monthData[curMonth]+"-"+ dayData[curDay]
                    var tmp = Date.fromLocaleString(locale, dateTimeString, "yyyy-MM-dd").toString().slice(0,2)
        ...
        Button
        {
            ...
            onClicked:{
                console.log("取消   " + yearData[curYear] + " " + monthData[curMonth] + " " + dayData[curDay])
                ikDateBackground.visible = false
            }
        }
        Button
        {
            ...
            //        iconSource:{ source: "qrc:///res/image/setImage/基本.png"}
            onClicked:{
                console.log("确认  " + yearData[curYear]  + " " + monthData[curMonth] + " " + dayData[curDay])

                currentData = yearData[curYear] + "-" +monthData[curMonth]+"-"+dayData[curDay]
                ikDateBackground.visible = false
            }
        }
        ...
        Rectangle{
            id:linea
            x:0
            y: month.y + month.height /5 *2///计算不对
            width: parent.width
            height: 2
            color: "#cccccc"
        }
