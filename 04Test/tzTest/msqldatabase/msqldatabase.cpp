...
int msqldatabase::open()
{
    ...
    if(!tableList.contains("irisalgo"))
    {
        ...
        create_sql.append("updatetime datetime,");
        create_sql.append("memo text,");
        create_sql.append("pupilrow Integer not null DEFAULT 0,");
        create_sql.append("pupilcol Integer not null DEFAULT 0,");
        create_sql.append("pupilradius Integer not null DEFAULT 0,");
        ...
        create_sql.append("spoofvalue Integer not null DEFAULT 0,");
        create_sql.append("interlacevalue Integer not null DEFAULT 0,");
        create_sql.append("qualitylevel Integer not null DEFAULT 0,");
...
/*********************************
 ...
 返  回 值:成功返回0,失败返回错误码
**********************************/
int msqldatabase::insertIrisInfo(QList<PIRISUserInfo> &infoList)
{
...
/*********************************
 ...
 返  回 值:成功返回0,失败返回错误码
**********************************/
int msqldatabase::insertEnrollPara(QList<PIRISEnrPara>& enrollInfoList)
{
    if(database.isOpen())
    {
            ...
            else
            {
                ...
                if( value == 0)
                {
                    QString insert_data = "INSERT INTO irisalgo (irisdataid,personid,regtime,eyeflag,groupid,memo,pupilrow,pupilcol,pupilradius,irisrow,iriscol,irisradius,\
                            ...
                            VALUES (:irisdataid,:personid,:regtime,:memo,:pupilrow,:pupilcol,:pupilradius,:irisrow,:iriscol,:irisradius,\
                                    :focusscore,:percentvisible,:spoofvalue,:interlacevalue,:qualitylevel,:qualityscore)";
                    ...
                    sql_query.bindValue(":regtime",enrollInfoList.at(i)->m_eyeflag);
                    sql_query.bindValue(":regtime",enrollInfoList.at(i)->m_groupid);
                ...
                else
                {
                    ...
                    return E_MSQL_IRISALGO_INSERT_DATABEING_INSERT_ERROR;
                }
            }
        }
    }
...
/*********************************
 ...
 返  回 值:成功返回0,失败返回错误码
**********************************/
int msqldatabase::addPersonInfo(const QList<PDBPersonInfo>& personInfoList)
{
    if (!database.isOpen())
    {
        return E_MSQL_OPEN_ERROR;
    ...
    for(int i = 0;i < personInfoList.length();i++)
    {
        ...
        /*****   如果不存在记录则插入  *****/
        QString insert_data = "INSERT INTO person (personid,name,id,superid,sex,cardid,idcard,memo,profession,hasirisdata,\
                address,password,updatetime)\
                VALUES (:personid,:name,:id,:superid,:sex,:cardid,:idcard,:memo,:profession,:hasirisdata,\
                        :address,:password,:updatetime)";
...
/*********************************
 ...
 返  回 值:成功返回0,失败返回错误码
**********************************/
int msqldatabase::updatePersonInfo(const QList<PDBPersonInfo>& personInfoList)
{
    if(database.isOpen())
    {
        return E_MSQL_OPEN_ERROR;
    ...
    for(int i = 0;i < personInfoList.length();i++)
    {
        ...
        if(!sql_query.exec())
        {
            ...
            QString insert_data = "UPDATE person SET personid = :personid,name = :name,id = :id,superid = :superid,sex = :sex,\
                    cardid = :cardid,idcard = :idcard,memo = :memo,profession = :profession,\
...
//        nameList.clear();
        ...
        QSqlQuery sql_query;
        QString query_data = "select id,name from person"; //查询数据
...
int msqldatabase::getPersonByCard(QString cardid, DBPersonInfo& personInfo)
{
    ...
    QSqlQuery sql_query;
    QString query_data;
    query_data = "SELECT * FROM person where cardid=:cardid";
    sql_query.prepare(query_data); //查询
