...
public:
    ...
    //irisdatabase数据表：    
    int insertIrisInfo(QList<PIRISUserInfo>& infoList);//插入虹膜信息
    ...
    //irisalgo数据表:
    int insertEnrollPara(QList<PIRISEnrPara>& enrollInfoList);//插入虹膜注册参数
    ...
    int updatePersonInfo(const QList<PDBPersonInfo>& personInfoList);//修改用户信息
