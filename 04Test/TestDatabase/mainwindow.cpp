  ...
  //,ov5640framesThread(nullptr)
{
    ...
    connect(this, SIGNAL(sig_loadPersonIris()), sqlManageObj, SLOT(slot_loadPersonIris()), Qt::AutoConnection);

    connect(this, SIGNAL(sig_insertIrisParas()), sqlManageObj, SLOT(slot_insertIrisParas()));

    myThread.start();

    emit sig_insertIrisParas();
...
void MainWindow::on_PB_addFeature_clicked()
{
    int count = 50;
    emit sig_insertPersonIris(50);
    appendInfo(QString("begin insert person and iris totally %1").arg(count));
}

void MainWindow::on_PB_addPerson_clicked()
{
    int count = 50;
    emit sig_insertPerson(50);
    appendInfo(QString("begin insert person totally %1").arg(count));
}

void MainWindow::on_PB_loadFeature_clicked()
{
    emit sig_loadPersonIris();
