...
template<typename dump> static void CopyColumn(const unsigned char* _src, size_t src_step, unsigned char* _dst, size_t dst_step, int len, size_t elem_size);
template<typename dump> static void CopyFrom2Columns(const unsigned char* _src, size_t src_step, unsigned char* _dst0, unsigned char* _dst1, int len, size_t elem_size);
template<typename dump> static void CopyTo2Columns(const unsigned char* _src0, const unsigned char* _src1, unsigned char* _dst, size_t dst_step, int len, size_t elem_size);
template<typename dump> static void ExpandCCS(unsigned char* _ptr, int n, int elem_size);
...
{
    assert(typeid(float).name() == typeid(_Tp).name());
    assert(chs1 == 1 || chs1 == 2 || chs2 == 1 || chs2 == 2);
    ...
    int elem_size = (int)sizeof(_Tp), complex_elem_size = elem_size * 2;
	int factors[34];
	...
	if (!inv && src0.channels == 1 && (flags & DFT_COMPLEX_OUTPUT)) {
		...
		if (dst.empty()) {
            ...
            assert(src0.rows == dst.rows && src0.cols == dst.cols);
		}
	...
	if (src0.cols == 1 && nonzero_rows > 0) {
        assert("This mode (using nonzero_rows with a single-column matrix) breaks the function's logic, so it is prohibited.\n"
	...
	for (;;) {
        ...
        buf = (unsigned char*)malloc(sz + 32);
        memset(buf, 0, sz + 32);
        ptr = buf;
//		ptr = (uchar*)buf;
//		buf.allocate(sz + 32);
		...
		if (!spec) {
			...
			itab = (int*)ptr;
            ptr = (unsigned char*)(((size_t)(ptr + len*sizeof(int)) + 16 - 1) & ~(size_t)(16 - 1));
		...
		if (stage == 0) {
			...
			for (i = 0; i < nonzero_rows; i++) {
                const unsigned char* sptr = src.ptr(i);
                unsigned char* dptr0 = dst.ptr(i);
            ...
            const unsigned char* sptr0 = src.ptr();
            unsigned char* dptr0 = dst.ptr();
			buf0 = ptr;
			ptr += len*complex_elem_size;
			buf1 = ptr;
			ptr += len*complex_elem_size;
...
static void CopyColumn(const unsigned char* _src, size_t src_step, unsigned char* _dst, size_t dst_step, int len, size_t elem_size)
{
	int i, t0, t1;
...
template<typename dump>
static void CopyFrom2Columns(const unsigned char* _src, size_t src_step, unsigned char* _dst0, unsigned char* _dst1, int len, size_t elem_size)
{
	int i, t0, t1;
	const int* src = (const int*)_src;
	int* dst0 = (int*)_dst0;
...
template<typename dump>
static void CopyTo2Columns(const unsigned char* _src0, const unsigned char* _src1, unsigned char* _dst, size_t dst_step, int len, size_t elem_size)
{
	int i, t0, t1;
...
template<typename dump>
static void ExpandCCS(unsigned char* _ptr, int n, int elem_size)
{
	int i;
