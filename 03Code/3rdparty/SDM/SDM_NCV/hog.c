...
VlHog * vl_hog_new (VlHogVariant variant, vl_size numOrientations, vl_bool transposed)
{
  ...
  self->orientationX = (float*)malloc(sizeof(float) * (size_t)self->numOrientations) ;
  self->orientationY = (float*)malloc(sizeof(float) * (size_t)self->numOrientations) ;
  ...
  self->permutation = (vl_index*)malloc((size_t)self->dimension * sizeof(vl_index)) ;
  switch (self->variant) {
    case VlHogVariantUoctti:
      for(o = 0 ; o < (signed)self->numOrientations ; ++o) {
   ...
   */
  self->glyphs = (float*)calloc((size_t)(self->glyphSize * self->glyphSize * self->numOrientations), sizeof(float)) ;
...
vl_hog_prepare_buffers (VlHog * self, vl_size width, vl_size height, vl_size cellSize)
{
  ...
  if (self->hog &&
    ...
    memset(self->hog, 0, (size_t)(sizeof(float) * hogWidth * hogHeight * self->numOrientations * 2)) ;
    memset(self->hogNorm, 0, (size_t)(sizeof(float) * hogWidth * hogHeight)) ;
    return ;
  }
  ...
  self->hog = (float*)calloc((size_t)(hogWidth * hogHeight * self->numOrientations * 2), sizeof(float)) ;
  self->hogNorm = (float*)calloc((size_t)(hogWidth * hogHeight), sizeof(float)) ;
  self->hogWidth = hogWidth ;
  self->hogHeight = hogHeight ;
}
  ...
  for (y = 1 ; y < (signed)height - 1 ; ++y) {
    for (x = 1 ; x < (signed)width - 1 ; ++x) {
      ...
      if (self->useBilinearOrientationAssigment) {
        ...
        float angle0 = acosf((float)VL_MIN(orientationWeights[0], 1.0f)) ;
      ...
      for (o = 0 ; o < 2 ; ++o) {
        ...
        hx = (float)((x + 0.5) / cellSize - 0.5) ;
        hy = (float)((y + 0.5) / cellSize - 0.5) ;
        binx = vl_floor_f(hx) ;
        biny = vl_floor_f(hy) ;
        wx2 = hx - binx ;
        wy2 = hy - biny ;
