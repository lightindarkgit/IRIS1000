...
// extern const int IK1000_ENROLL_FEATURE_SIZE; // = 1000;

const int g_SuperCount =  120;                           //超级管理员的数量
const int g_SuperHalf  = g_SuperCount/2;                             //数量的一半
const int g_FeatureSize = 1000;                            //特征长度
enum class ControlType:short {Add,Del,Update};           //枚举类，内存中特征处理的类型
enum class EyeFlag:short{All,Left,Right,Either};    //枚举：左眼、右眼、全部、任意眼
...
//    uuid_t  FeatureID;
    ...
    std::string UserID;       //用户ID，在AI1000系统中是一个唯一的id，但不是uuid
    std::string Name;

    std::string Depart;
    std::string Sex;

    bool IsSuper;       //  是否为管理员，true是管理员
...
//	uuid_t ud;   //特征ID
    std::string ud;   //特征ID
    EyeFlag  TypeControl;
    unsigned char FeatureData[g_FeatureSize];
...
{
//    uuid_t PeopleID;                   //人员ID，uuid
    std::string PeopleID;               //人员ID，uuid
...
//	std::list<GUID> RightFeature;      //左眼特征ID数组
    ...
    std::list<std::string> LeftFeature;       //右眼特征ID数组
    std::list<std::string> RightFeature;      //左眼特征ID数组
