...
void UuidUnparse(const uuid_t ud,char * udout)
{
    return uuid_unparse(ud,udout);
}

std::string StringFromUuid(const uuid_t ud)
{
    char buf[64];
    UuidUnparse(ud,buf);
...
void IrisData::InitSuperData()
{
	...
	//开辟管理员特征，固定
	//指定左眼特征指针头和当前位置指针
    _pLHead = _pLFeatureData = _pFeatureData =_pRFeatureData = new unsigned char[g_SuperCount * g_FeatureSize];
 ...
 *  修改时间：
 *****************************************************************************/
bool IrisData::GetIrisFeature(EyeFlag flag,unsigned char *&pIrisData,int &size)
{
	Exectime etm(__FUNCTION__);
	bool bRet = true;
	//分别获取左右眼的特征
    if ((flag == EyeFlag::Left) && (_countLeft > 0))
	{
		//此处的锁移到显示的加锁和解锁函数中
		size = _countLeft;
		pIrisData = _pLFeatureHead;
	}
    else if ((flag == EyeFlag::Right) && (_countRight > 0))
	{
		size = _countRight;
 ...
 *  修改时间：
 *****************************************************************************/
bool IrisData::SelectInfoFromFeature(int index,PeopleInfo &name,EyeFlag type)
{
	Exectime etm(__FUNCTION__);
	bool bRet = false;
	//是否判断大于整体索引值的情况
    ...
    name.UserID = pTemp.p->UserID;
	name.Depart = pTemp.p->Depart;
    name.IsSuper = pTemp.p->IsSuper;
    name.PersonID = pTemp.p->PeopleID;
    name.FeatureID = pTemp.FeatureID;
    // memmove(name.PersonID,pTemp.p->PeopleID,16);
...
// 通过虹膜UUID获取人员
bool IrisData::GetPeopleInfoByIrisFeature(const std::string& irisuuid,PeopleInfo& personinfo)
{
	bool bret = false;
	...
	while(plastnodeL != _pPosCNode)
	{
        ...
        if (irisuuid==plastnodeL->FeatureID)
		{
			//目前仅返回人员姓名和ID
			personinfo.Name = plastnodeL->p->Name;
			personinfo.CardID = plastnodeL->p->CardID;
            personinfo.UserID = plastnodeL->p->UserID;
			personinfo.Depart = plastnodeL->p->Depart;
            personinfo.IsSuper = plastnodeL->p->IsSuper;
            personinfo.PersonID = plastnodeL->p->PeopleID;
            personinfo.FeatureID = plastnodeL->FeatureID;

//            memmove(personinfo.PersonID,plastnodeL->p->PeopleID,16);
//			memmove(personinfo.FeatureID,plastnodeL->FeatureID,16);
	...
	while(plastnodeR != _pPosRCNode)
	{
        ...
        if (irisuuid==plastnodeR->FeatureID)
		{
			//目前仅返回人员姓名和ID
			personinfo.Name = plastnodeR->p->Name;
			personinfo.CardID = plastnodeR->p->CardID;
            personinfo.UserID = plastnodeR->p->UserID;
			personinfo.Depart = plastnodeR->p->Depart;
            personinfo.IsSuper = plastnodeR->p->IsSuper;
            personinfo.PersonID = plastnodeR->p->PeopleID;
            personinfo.FeatureID = plastnodeR->FeatureID;

//            memmove(personinfo.PersonID,plastnodeR->p->PeopleID,16);
 ...
 *  修改时间：
 *****************************************************************************/
bool IrisData::InsertFeatureData(const std::string& featureID,
                                 std::shared_ptr<PeopleNode> &pPnode,
                                 EyeFlag flag,
                                 const unsigned char * pIrisData)
{
	Exectime etm(__FUNCTION__);
	bool bRet = false;
//	GUID gd;
//	memmove(gd.ud,featureID,16);
    std::string gd = featureID;
	...
	//建立特征ID和特征序列数组的映射
	//left eye
    if (flag == EyeFlag::Left)
	{
		pPnode->LeftFeature.push_back(gd);                               //将左眼的特征ID加入到人员的左眼队列
        memmove(_pPosFeature,pIrisData,g_FeatureSize);                    //拷贝特征
        _pPosCNode->FeatureID = featureID;
        // memmove(_pPosCNode->FeatureID,featureID,g_UuidLen);              //拷贝特征ID到映射结点
		_pPosCNode->p = pPnode;
		//建立特征ID和数组索引的映射
        _mapFeature[featureID] = _curPlace;

		//控制位置－始终在尾节点位置
		_curPlace++;                                                     //注意索引与下面特征和人员的数组的对应关系
        _pPosFeature += g_FeatureSize;
		_pPosCNode++;
	...
	else   //right eye
	{
        ...
        memmove(_pPosRFeature,pIrisData,g_FeatureSize);                   //拷贝特征
        _pPosRCNode->FeatureID = featureID;
        // memmove(_pPosRCNode->FeatureID,featureID,g_UuidLen);             //拷贝特征ID到映射结点
		_pPosRCNode->p = pPnode;
		//建立特征ID和数组索引的映射
        _mapFeature[featureID] = _curRPlace;

		//控制位置
		_curRPlace++;                                                    //注意索引与下面特征和人员的数组的对应关系
        _pPosRFeature += g_FeatureSize;
		_pPosRCNode++;
		_countRight++;
	}
 ...
 *  修改时间：
 *****************************************************************************/
int IrisData::DeleteFeatureData(const std::string& featureID,EyeFlag flag)
{
	Exectime etm(__FUNCTION__);
...
//        std::string sKey = StringFromUuid(featureID);
//        std::string sMovKey = StringFromUuid(plastnodeL->FeatureID);
         std::string sKey = featureID;
         std::string sMovKey = plastnodeL->FeatureID;
        //处理人员映射
...
//        uuid_copy((_pLCNodeHead + index)->FeatureID, plastnodeL->FeatureID);
        (_pLCNodeHead + index)->FeatureID =  plastnodeL->FeatureID;
       ...
       unsigned char* plastfeatureL = (_pPosFeature == _pLFeatureHead) ? _pPosFeature : (_pPosFeature - g_FeatureSize);
        memmove(_pLFeatureHead+ g_FeatureSize*index,plastfeatureL, g_FeatureSize);
        memset(plastfeatureL, 0, g_FeatureSize);
        _pPosFeature -= g_FeatureSize;                                           //退后512个字节
	...
	else
	{
		...
		//得到当前和最后一个，最后倒入到当前，并清空最后原来的内存空间
//        std::string sKey = StringFromUuid(featureID);
//        std::string sMovKey = StringFromUuid(plastnodeR->FeatureID);

        std::string sKey = featureID;
        std::string sMovKey = plastnodeR->FeatureID;
        ...
        (_pRCNodeHead + index)->FeatureID =  plastnodeR->FeatureID;
		...
		//处理特征－－每512个字节对就一个index
        unsigned char* plastfeatureR = (_pPosRFeature == _pRFeatureHead) ? _pPosRFeature : (_pPosRFeature - g_FeatureSize);
        memmove(_pRFeatureHead+ g_FeatureSize* index,plastfeatureR, g_FeatureSize);
        memset(plastfeatureR, 0, g_FeatureSize);
        _pPosRFeature -= g_FeatureSize;                                                                  //退后512个字节
 ...
 *  修改时间：
 *****************************************************************************/
bool IrisData::GetSuperFeature(EyeFlag flag,unsigned char*&pIrisData,int &size)
{
    Exectime etm(__FUNCTION__);
	bool bRet = true;

    if ((flag == EyeFlag::Left) && (_superLCount > 0))
	{
		pIrisData = _pLHead;
		size = _superLCount;
	}
    else if ((flag == EyeFlag::Right)&&(_superRCount > 0))
	{
		pIrisData = _pRHead ;
		size = _superRCount;
	}
...
bool IrisData::ReleaseSuperData(void)
{
    ...
    memset(_pFeatureData, 0, g_SuperCount*g_FeatureSize);                //初始化缓冲区
 ...
 *  修改时间：
 *****************************************************************************/
bool IrisData::BuildSuperData(EyeFlag flag,unsigned char *pIrisData,PeopleNode &pnData)
{
	Exectime etm(__FUNCTION__);
    ...
    if (EyeFlag::Left == flag)
	{
        ...
        memmove(_pLFeatureData,pIrisData,g_FeatureSize);
        _pLFeatureData+=g_FeatureSize;
		_superLCount++;
	}
	else
	{
        ...
        memmove(_pRFeatureData,pIrisData,g_FeatureSize); //申请了120*512空间，但首地址从60*512开始；dj
        _pRFeatureData += g_FeatureSize;
		_superRCount++;
	}
