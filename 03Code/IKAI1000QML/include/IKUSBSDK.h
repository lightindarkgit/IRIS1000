...
#define IKUSBSDK_IKEMBX00_NO
#define IKUSBSDK_OV580_NO
#define IKUSBSDK_OV580L_NO
#define IKUSBSDK_UVC_NO_NO
#define IKUSBSDK_IMX132_NO
#define IKUSBSDK_AIO4060_NO
#define IKUSBSDK_21BS35_NO
#define IKUSBSDK_OV2710
#define IKUSBSDK_21BS40_NO
#define IKUSBSDK_21BL50_NO
#define IKUSBSDK_21BL55_NO
...
// 用#error控制
// 此数值必须与IKAlgoAPI.h中定义的IKALG_Iris_Enr_CodeLen一致
#define IK_ENROLL_FEATURE_SIZE 1000
// 此数值必须与IKAlgoAPI.h中定义的IKALG_Iris_Rec_CodeLen一致
// const int IK_IDEN_FEATURE_SIZE = 1024; // 1536;
#define IK_IDEN_FEATURE_SIZE  1536
...
//比对过程中的输出结构
typedef struct IKSDKMatchIndexStruct
{
    int matchIndex; //表示当前已经符合注册要求的图像数目
    int matchScore;
} IKSDKMatchIndex, *pIKSDKMatchIndex;

typedef struct IKSDKMatchInfoStruct
{
    unsigned char* codeList;
    int codeNum;
    //pIKSDKMatchIndex OutIndex;
} IKSDKMatchInfo, *pIKSDKMatchInfo;
...
const int E_USBSDK_PERSON_DUPLI = E_USBSDK_ERROR_BASE - 2202;      //比对失败,人员重复
...
/*****************************************************************************
*                         开始虹膜注册
*  函 数 名：IKUSBSDK_StartIrisEnrollWithInfo
*  功    能：带查重功能的虹膜注册
*  说    明：

*  参    数：enrollFeature：输出参数，注册得到的虹膜特征
*			 enrollInfo：输出参数，注册得到的左眼、右眼虹膜图像个数及图像
*			 irisMode：输入参数，指定需要指定哪只眼睛进行注册
*			 resultFlag：输出参数，左眼、右眼虹膜注册成功标志
*			 algConfig：输入参数，算法控制参数
*			 MatchInofL：已注册的注册特征。
...
*  修改时间：
*****************************************************************************/
__IK_USBSDK_DLL_ int __IK_STYLE_CALL IKUSBSDK_StartIrisEnrollWithInfo(OUT IKSDKIrisInfo *enrIrisL, IN int numEnrL, OUT IKSDKIrisInfo *enrIrisR, IN int numEnrR, IN IKIrisMode irisMode, OUT IKResultFlag &resultFlag, IN IKAlgConfigStruct *algConfig,
    IN IKSDKMatchInfo *MatchInofL, OUT pIKSDKMatchIndex MatchIndexL, IN IKSDKMatchInfo *MatchInofR, OUT pIKSDKMatchIndex MatchIndexR);
/*****************************************************************************
*                         开始人脸注册
...
/*****************************************************************************
*                         开启红外灯
*  函 数 名：IKUSBSDK_StartIRLED
*  功    能：开启红外灯
*  说    明：在开始虹膜采集之前，需要开启红外灯
*  参    数：
*
*
*  返 回 值：0：开启成功
*    		<0：开启失败,根据返回值可以查询失败原因,详见函数调用错误码
*  创 建 人：lizhl
*  创建时间：2017-05-12
*  修 改 人：
*  修改时间：
*****************************************************************************/
__IK_USBSDK_DLL_ int __IK_STYLE_CALL IKUSBSDK_StartIRLED();
